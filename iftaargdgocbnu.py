...
if st.button('Start Selection'):
    ...
    # Blink the names and select each one sequentially
    for i in range(len(selected_students)):
        # "Blink" effect: Show all names and clear the screen before showing the selection
        for _ in range(5):  # Repeat blinking effect
            st.write(" ")
            time.sleep(0.5)
        ...
        # Select one student and show it
        st.write(f"Selected Student: {selected_students[i]}")
